...
class Clean_File:
    ...
    def clean_df(self):
        ...
        self.df['Description'] = self.df[[1,2]].apply(lambda x: self.extract_description(x[1], x[2]), axis = 1 )
        ...
        # example "1.00 EA" => "EA"
        self.df['Quantity Type'] = self.df[9].apply(lambda x: str(x).split(" ")[-1])

        # Make "Net Quantity" column
        # example "1.00 EA" => "1.00"
        self.df['Net Quantity'] = self.df[9].apply(lambda x: str(x).split(" ")[0])
        ...
        self.df = pd.concat([self.df[self.df.index == -1], self.df[self.df.index != -1]])
